...
// Maze funcs ------------------------------------
func (с *ConsoleWriter) PrintMaze(path string, buf io.Writer, maze *domain.Maze) error {
	if path != "" {
...
func (c *ConsoleWriter) PrintPath(output string, path *domain.Path, maze *domain.Maze) error {
	p := path.GetPath()

	for i, cell := range p {
		switch i {
		case 0:
			_ = maze.SetCell(cell.Row, cell.Col, domain.CellStart)
		case len(p) - 1:
			_ = maze.SetCell(cell.Row, cell.Col, domain.CellEnd)
		default:
			_ = maze.SetCell(cell.Row, cell.Col, domain.CellPath)
		}
	}
