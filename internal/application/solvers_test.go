...
func TestDijkstraSolver(t *testing.T) {
	...
	for _, tc := range tests {
		tc := tc
		t.Run("Dijkstra_solver_check", func(t *testing.T) {

			maze := dfs.Generate(tc.rows, tc.cols)
			mazeScheme := maze.GetScheme()

			start := domain.NewCell(tc.startX, tc.startY)
			end := domain.NewCell(tc.endX, tc.endY)

			path := dijkstra.Solve(maze, start, end)
			...
			if checkPath(mazeScheme, maze, realPath) {
...
func checkPath(scheme [][]domain.CellType, maze *domain.Maze, path []domain.Cell) bool {
	var prev domain.Cell
	if len(path) == 0 {
		return true
	}

	for i, p := range path {
		cell := domain.CellPath
		switch i {
		case 0:
			cell = domain.CellStart
		case len(path) - 1:
			cell = domain.CellEnd
		}

		c, _ := maze.GetCell(p.Row, p.Col)
		if !((scheme[p.Row][p.Col] == domain.CellEmpty) && (c == cell)) {
			return false
		}
