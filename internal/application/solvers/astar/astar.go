...
func (a *AStar) Solve(maze *maze.Maze, start, end cell.Cell) *path.Path {
	pq := &support.PQAStar{}
	heap.Init(pq)
	...
	heap.Push(pq, support.AstarItem{Dist: 0, H: support.Heuristic(start, end), C: start})

	for pq.Len() != 0 {
		item := heap.Pop(pq).(support.AstarItem)
		...
		for _, way := range directions {
			...
			if oldDist, ok := dists[p]; (maze.InBorders(p.Row, p.Col)) &&
				...
				if v, _ := maze.GetCell(item.C.Row+way[0], item.C.Col+way[1]); v == cell.CellEmpty {
					dists[p] = item.Dist + 1
					prevs[p] = item.C
					heap.Push(pq, support.AstarItem{Dist: item.Dist + 1, H: support.Heuristic(p, end), C: p})
				}
